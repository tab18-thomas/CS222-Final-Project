...
// Methods
	...
	private void tryAttack() {
		...
		// Looping through to see if they want to attack and which territory to attack from. 
		//The condition is that they must have at least 4 troops
		for(Territory t: territories) {
			if(t.numTroops < 2) {
				continue;
			}
			if(t.numTroops > bestTroopCount) { // Going through and seeing which of the territories' number of troops is the most
		...
		if(attackFrom == null) {
			...
			for(Territory t: attackFrom.borderingTerritories(this)) {
				if(t.numTroops < bestTroopCount) { // going through and seeing which bordering territory has less than the attacking territory troops and has the least amount of troops 
					bestTroopCount = t.numTroops;
					defending = t; 
				}
			}
